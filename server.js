...
app.post("/api/subscribe", async (req, res) => {
  ...
  try {
    const result = await pool.query("INSERT INTO emails (email) VALUES ($1) ON CONFLICT (email) DO NOTHING RETURNING *", [email]);
...
async function sendThresholdEmail(user, indicatorName, crossedPrice, currentPrice, crossedTimestamp) {
  ...
  const subject = `【Parabolic】ドル円価格アラート: ${indicatorName.toUpperCase()} のしきい値達成`;
  const text = `
...
---
- 監視対象: USD/JPY
- トリガー指標: ${indicatorName.toUpperCase()}
...
`;
  const html = `
    ...
    <ul>
      ...
      <li><b>トリガー指標:</b> ${indicatorName.toUpperCase()}</li>
  ...
  try {
    ...
    for (const indicatorName in indicatorValues) {
      ...
      if (lastRelPosition !== secondLastRelPosition) {
        ...
        realTimeState.crossHistory[indicatorName] = { price: crossPrice, timestamp: crossTimestamp.toISOString() };
        stateChanged = true;

        console.log(
          `User ${user.id}: CLOSE(SMA1)-BASED cross detected for ${indicatorName} at price ${crossPrice} on interval ${userInterval}`
        );
        ...
        if (socketId) {
          ...
          const eventPayload = {
            message: `ドル円が ${indicatorName.toUpperCase()} を終値で${lastRelPosition === "above" ? "上抜け" : "下抜け"}しました！ (間隔: ${userInterval})`,
            price: crossPrice,
          ...
          // 接続はあるのに紐付かない時のデバッグ用
          // console.log(`No socketId found for user ${user.id} (not connected or session missing).`);
        }
      }
    }
  } catch (error) {
...
async function startPriceWatcher() {
  ...
  setInterval(async () => {
    try {
      ...
      /**
       * ✅ ここが重要修正
       * quote が落ちても return しない（＝クロス判定まで到達する）
       */
      ...
      for (const user of users) {
        ...
        if (currentPrice != null && Object.keys(realTimeState.crossHistory || {}).length > 0) {
          for (const indicatorName in realTimeState.crossHistory) {
            ...
            if (crossEvent) {
              const { price: crossedPrice } = crossEvent;
              const priceDifference = Math.abs(currentPrice - crossedPrice);
              if (parseFloat(user.x_value) > 0 && priceDifference > parseFloat(user.x_value)) {
                await sendThresholdEmail(user, indicatorName, crossedPrice, currentPrice, new Date(crossEvent.timestamp));
        ...
        try {
          const quotes = await fetchYahooChartQuotes(ticker, userInterval, period1, period2);
          const crossDetectionStateChanged = await monitorSma1Value(user, quotes, realTimeState, socketMap, io);
